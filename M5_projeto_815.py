...
questoes = {
'Apresentação': 'O Brasil nas olímpiadas',
...
if sb == 'Questão 1':
	st.subheader('1. Crie um DataFrame contendo apenas informações sobre atletas brasileiros. Exiba a quantidade de linhas desse novo DataFrame.')
	st.markdown('<hr>', unsafe_allow_html=True)
    
	'''
	Para a montagem do banco, fez-se a leitura do arquivo com o uso da biblioteca Pandas e, a priori, apenas se trocou todos os 'Nan' por 'Não-Medalhista', indicando que o referido atleta não recebeu medalha. Para tal, fez-se uso do método 'fillna', na coluna 'Medal'.
	'''
	'''
	Isso foi feito para facilitar a análise e contagem, sem precisar ficar chamando comandos que incluam os 'NaN' nas contagens.
	'''
...
if sb == 'Questão 2':
	st.subheader('2. Será que existe alguma correlação entre a altura do atleta e seu desempenho, para algum esporte? Faça essa análise separadamente para cada gênero, e conclua.')
	st.markdown('<hr>', unsafe_allow_html=True)
...
columns=['ID','Name','Sex','Age','Height','Weight', 'Team','NOC','Games','Year','Season','City'])
	...
	fig = px.box(x='Sport', y='Height', data_frame=br_f_5, color='Medal', title='Desempenho x Altura (F)', color_discrete_sequence=['Orange','Grey'])
	...
	st.markdown(f'<img src="data:image/gif;base64,{data_url}" alt="Feminino" width=800 height=400>', unsafe_allow_html=True)
...
columns=['ID','Name','Sex','Age','Height','Weight', 'Team','NOC','Games','Year','Season','City'])
	...
	fig = px.box(x='Sport', y='Height', data_frame=br_m_5, color='Medal', title='Desempenho x Altura (F)', color_discrete_sequence=['Orange','Grey'])
	...
	st.markdown(f'<img src="data:image/gif;base64,{data_url}" alt="Masculino" width=800 height=400>', unsafe_allow_html=True)
...
if sb == 'Questão 3':
	...
	fig.update_layout(annotations=[{'text':'13                  08                 22                 18                 09                 10', 'xref':'x', 'yref':'y', 'x':2.5, 'y':115, 'showarrow':False, 'ax': 0,    'ay': -80, 'font':dict(family="Nunito", size=16, color="#e67e22")}])
